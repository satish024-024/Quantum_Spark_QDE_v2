...
var GlobalContext = {
    ...
    init: function () {
        ...
        // init label renderer - with fallback for CSS2DRenderer
        try {
            if (typeof THREE.CSS2DRenderer !== 'undefined') {
                GlobalContext.labelRenderer = new THREE.CSS2DRenderer();
            } else {
                // Fallback: create a simple div-based label system
                ...
                console.warn('CSS2DRenderer not available, using fallback label system');
            }
        } catch (error) {
            console.warn('CSS2DRenderer initialization failed:', error);
            // Create fallback label renderer
            GlobalContext.labelRenderer = {
                domElement: document.createElement('div'),
                setSize: function(w, h) { 
                    this.domElement.style.width = w + 'px'; 
