...
// Import from global THREE object (CSS2DRenderer.js and OrbitControls.js use global THREE)
// import {
//     CSS2DRenderer
...
var GlobalContext = {
    ...
    init: function () {
        ...
        // init label renderer with compatibility fix
        if (typeof THREE.CSS2DRenderer !== 'undefined') {
            GlobalContext.labelRenderer = new THREE.CSS2DRenderer();
        } else {
            // Fallback for missing CSS2DRenderer
            console.warn('CSS2DRenderer not available, using fallback');
            GlobalContext.labelRenderer = {
        ...
        // init orbit controls with compatibility fix
        if (typeof THREE.OrbitControls !== 'undefined') {
            GlobalContext.controls = new THREE.OrbitControls(GlobalContext.camera, GlobalContext.labelRenderer.domElement);
            GlobalContext.controls.minDistance = diameter / 4;
            GlobalContext.controls.maxDistance = diameter * 2;
        } else {
            // Fallback for missing OrbitControls
            console.warn('OrbitControls not available, using fallback');
            GlobalContext.controls = {
                update: function() {
                    // No-op fallback
                }
            };
